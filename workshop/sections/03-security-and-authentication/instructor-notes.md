...
- **Lab 3.5 Step 4 / deploy path** — `apply-cert-blacklist.sh` only patches `tls-cert-blacklist-secret`. Trainees then run **one** deploy script for their path: `deploy-cluster-tls-mtls-blacklist.sh` (Path A) or `-helm.sh` (Path B). Do not `kubectl apply` the blacklist manifest on a Helm-managed release.
...
- **Lab 3.2 → 3.3 handoff** — trainees deploy mTLS directly (`deploy-cluster-tls-mtls*.sh`); `prepare-lab.sh 3.3` is instructor recovery only (`--skip-reset` default: validate + deploy + wait; no teardown).
