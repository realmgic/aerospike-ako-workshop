...
- Dim cluster **Running** (for meaningful collectinfo output):
...
tar -tzf /tmp/akoctl-lab/*.tar.gz | head -30
```
...
**Point:** `asadm collectinfo` = node/OS detail; `akoctl collectinfo` = cluster/operator/K8s detail.
